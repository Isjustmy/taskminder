    ...
    <div v-if="role === 'admin'">
      <div>
        <div class="ml-5 text-2xl font-bold flex">
          <h1 class="pt-2">List User Siswa</h1>
          <router-link :to="{ name: 'user_create' }" class="btn btn-success text-white ml-12">Tambah User
            Baru</router-link>
        </div>
        ...
        <div class="mt-10 px-2">
          <div class="flex mb-4">
            ...
            <select class="select select-bordered ml-4 w-[30%]" v-model="sortOrder" @change="sortUserList">
              <option>Urutkan Berdasarkan Nomor Absen</option>
              <option value="asc">Nomor Absen dari kecil ke besar</option>
              <option value="desc">Nomor Absen dari besar ke kecil</option>
            </select>
          </div>
          <div>
            <table class="table table-fixed">
              <thead class="text-black font-bold text-[16px] border border-black">
                <tr>
                  <th class="text-center text-wrap text-sm w-[5%] px-0 border border-black">Nomor Absen</th>
              ...
              <tbody>
                <!-- Conditionally render message when no class is selected -->
                <tr v-if="!isClassSelected">
                  <td colspan="6" class="text-center">Silahkan pilih kelas untuk menampilkan data siswa berdasarkan
                    kelas.</td>
                </tr>
                ...
                <!-- Conditionally render message when no user data is available -->
                <tr v-else-if="!loadingUsers && userApiData.length === 0">
                  <td colspan="6" class="text-center">Tidak ada data user.</td>
                </tr>
                <!-- Render user data when available -->
                <tr v-else v-for="(userData, index) in userApiData" :key="userData.id">
                  <td class="text-center border border-black text-[16px] px-1 py-2">{{ userData.nomor_absen || '-' }}
                  </td>
                  <td class="text-left border border-black text-[16px] px-1 py-2">{{ userData.name }}</td>
                  <td class="text-left overflow-x-auto border border-black text-[16px] px-2 py-2">{{ userData.email }}
                  </td>
                  <td class="text-center border border-black text-[16px] px-1 py-2 overflow-x-auto">{{
      userData.phone_number }}</td>
                  <td class="text-center border border-black text-[16px] px-1 py-2">{{ userData.student_class ?
      userData.student_class.class : '-' }}</td>
                  <td class="border border-black text-[16px] px-1 py-2">
                    <button type="button" class="btn btn-error ml-2"
                      @click="confirmDeleteUser(userData.id)">Hapus</button>
                  </td>
                </tr>
              </tbody>
            </table>
              ...
              <div class="modal-box">
                <form method="dialog">
                  <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2"
                    @click.prevent="closeDeleteUserModal">✕</button>
                </form>
                ...
                <div class="modal-action">
                  <form method="dialog">
                    <button v-if="!loadingDelete" class="btn btn-success hover:bg-green-300 mr-4"
                      @click.prevent="deleteUserData">Ya, Hapus</button>
                    ...
                    <button class="btn btn-error hover:bg-red-300" @click.prevent="closeDeleteUserModal">Batal</button>
                  </form>
                </div>
              </div>
            </dialog>
...
export default {
  ...
  methods: {
    ...
    sortUserList() {
      if (this.sortOrder === 'asc') {
        this.userApiData.sort((a, b) => (a.nomor_absen > b.nomor_absen) ? 1 : -1);
    ...
    updateClassSelection() {
      this.isClassSelected = this.selectedClass !== null && this.selectedClass !== "Pilih Kelas";
  ...
  created() {
    if (this.userData) {
      ...
      this.role = this.userData.roles && this.userData.roles.length > 0 ? this.userData.roles[0] : ''
      this.fetchClassData(),
        this.selectedClass = "Pilih Kelas",
        this.sortOrder = "Urutkan Berdasarkan Nomor Absen"
    } else {
