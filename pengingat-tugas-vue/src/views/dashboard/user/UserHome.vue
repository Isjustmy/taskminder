        ...
        <div class="flex">
          <label class="form-control w-full max-w-xs mb-5">
            <input
              type="text"
              v-model="searchQuery"
              class="form-control input input-bordered border-slate-500 w-full max-w-xs focus:border-black"
          ...
          <div>
            <select
              ...
              class="text-sm border border-black rounded-lg w-30 h-10 ml-5 mt-1"
            >
          ...
          <div>
            <select
              v-model="selectedIdOrder"
              @change="applyIdFilter"
              class="text-sm border border-black rounded-lg w-30 h-10 ml-5 mt-1"
            >
              <option value="">Urutkan Berdasarkan ID</option>
              <option value="asc">ID Kecil ke Besar</option>
              <option value="desc">ID Besar ke Kecil</option>
            </select>
          </div>
        </div>
        <table class="table table-fixed">
          ...
          <tbody>
            <tr v-if="loadingUsers">
              <td colspan="8" class="text-center py-4">Memuat...</td>
            </tr>
            <!-- Use v-for to loop through the filtered user data -->
            <tr v-else v-for="userData in filteredUserData" :key="userData.id">
...
export default {
  ...
  watch: {
    searchQuery: function (newSearchQuery) {
      // Pastikan searchQuery telah diubah dengan benar
      console.log('New search query:', newSearchQuery)

      // Pastikan data pengguna sudah ada sebelum mencoba memfilter
      if (this.userApiData.data) {
        this.filteredUserData = this.filterUserData(newSearchQuery) // Panggil filterUserData sebagai fungsi dengan parameter baru
        console.log('Filtered user data:', this.filteredUserData)
      }
    }
  },
  computed: {
  ...
  methods: {
    applyIdFilter() {
      ...
      // Filter berdasarkan peran terlebih dahulu
      if (this.selectedRole) {
        filteredData = this.userApiData.data.filter((userData) => {
          return userData.roles.some((role) => role.name === this.selectedRole.toLowerCase())
        })
      } else {
        filteredData = this.userApiData.data || []
      }

      // Kemudian, filter berdasarkan search query
      filteredData = filteredData.filter((userData) => {
        return userData.name.toLowerCase().includes(this.searchQuery.toLowerCase())
      })

      // Terakhir, urutkan data berdasarkan ID sesuai dengan urutan yang dipilih
      if (this.selectedIdOrder === 'asc') {
        filteredData = filteredData.sort((a, b) => a.id - b.id)
      } else if (this.selectedIdOrder === 'desc') {
    ...
    async fetchUserData() {
      ...
      if (response.status === 200) {
        ...
        this.filteredUserData = this.userApiData.data || [] // Initialize filteredUserData with all user data
        this.applyRoleFilter() // Memanggil metode applyRoleFilter
    ...
    filterUserData(newSearchQuery) {
      if (!newSearchQuery && !this.selectedRole) {
        return this.userApiData.data || []
      }

      const lowerCaseSearchQuery = newSearchQuery.toLowerCase()
      const filteredData = (this.userApiData.data || []).filter((userData) => {
        // Filter berdasarkan search query
        const matchesSearchQuery = userData.name.toLowerCase().includes(lowerCaseSearchQuery)

        // Filter berdasarkan role
        const matchesRole =
          !this.selectedRole ||
          userData.roles.some((role) => role.name === this.selectedRole.toLowerCase())
    ...
    applyRoleFilter() {
      if (this.searchQuery) {
        // Jika terdapat pencarian yang aktif, terapkan filter role pada hasil pencarian
        this.filteredUserData = this.filteredUserData.filter((userData) => {
          return userData.roles.some((role) => role.name === this.selectedRole.toLowerCase())
        })
      } else {
        // Jika tidak ada pencarian yang aktif, terapkan filter role pada seluruh data pengguna
        if (this.selectedRole) {
          this.filteredUserData = this.userApiData.data.filter((userData) => {
            return userData.roles.some((role) => role.name === this.selectedRole.toLowerCase())
          })
        } else {
          this.filteredUserData = this.userApiData.data || []
        }
      }

      // Jika tidak ada data yang ditemukan, tampilkan pesan "Data tidak ditemukan"
      if (this.filteredUserData.length === 0) {
        this.filteredUserData.push({ id: null, name: 'Data tidak ditemukan' })
      }
    },
