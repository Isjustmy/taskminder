<template>
  <div>
    <div class="flex">
      ...
      <button
      class="btn ml-20 mt-3"
      :class="{ 'btn-disabled text-dark': isButtonDisabled, 'btn-error text-white': !isButtonDisabled }"
      @click="isButtonDisabled || openDeleteModal()"
  >
      ...
      <div class="w-1/2">
        <div>
          ...
          <div
            ...
            <img v-if="isImageFile(additionalData.file_path)" :src="additionalData.file_path" alt="File Icon"
      ...
      <div class="w-1/2">
        <div>
          ...
          <input type="text" class="input input-bordered w-full max-w-xs overflow-x-auto mt-5"
            :value="additionalData.link || 'Tidak ada link submit'" readonly />
        </div>
        <div class="flex mt-8">
    ...
    <dialog id="file-modal" class="modal">
      <div class="modal-box">
        <span style="font-size: 14px; cursor: pointer; position: absolute; top: 10px; right: 10px">
          <a :href="additionalData.file_path" target="_blank" class="btn btn-primary mr-5 text-white">Buka Gambar di Tab
            Baru</a>
          <button @click="closeFileModal()" class="btn btn-neutral text-white">Tutup</button>
        </span>
        <img v-if="isImageFile(additionalData.file_path)" :src="additionalData.file_path" alt="Full Image"
          class="mt-10" />
        <template v-else>
          <a :href="additionalData.file_path" target="_blank" class="btn btn-primary">Download File</a>
        </template>
      </div>
    </dialog>
...
export default {
  ...
  computed: {
    ...
    isButtonDisabled() {
      return this.loadingData || this.loadingDelete || this.additionalData.is_submitted === 0
    }
  },
  methods: {
    goBack() {
