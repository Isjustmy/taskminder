    ...
    <div class="flex-1 h-full max-w-4xl mx-auto bg-white rounded-lg shadow-2xl">
      <div class="flex flex-col md:flex-row">
        ...
        <div class="h-32 md:h-auto md:w-1/2 flex items-center justify-center">
          <img class="object-cover mx-auto w-[70%] h-[70%]" src="../assets/taskminder_logo.png" alt="img" />
        </div>
        <form class="flex items-center justify-center p-6 sm:p-12 md:w-1/2" @submit.prevent="login">
          <div class="w-full">
            ...
            <div>
              ...
              <input id="email" name="email" type="email" autocomplete="email" required placeholder="seseorang@mail.com"
            ...
            <button v-if="!loading" @click="login" type="submit"
            ...
            <h1 class="text-blue-600 text-sm underline">
              <router-link :to="{ name: 'register' }">Belum Punya Akun? Klik Disini Untuk Membuat Akun</router-link>
            </h1>
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    sendToken(userToken) {
      const parseData = Cookies.get('userData');
      ...
      try {
        const response = Api.post('/api/user/storeTokenFcm', userTokenData);
        if (response.status === 200) {
          const toast = useToast();
          toast.success('Berhasil Mengirim Data Token FCM User', {
            timeout: 2000,
        ...
        if (error.response.status === 500) {
          console.log(`Terjadi Kesalahan dalam pengiriman token FCM: ${error.response.message}`);
        }
      }
    },
    async login() {
