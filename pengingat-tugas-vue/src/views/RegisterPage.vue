    ...
    <div class="flex-1 h-full max-w-full mx-auto bg-white rounded-lg shadow-xl">
      <div class="flex flex-col md:flex-row">
        ...
        <div class="h-32 md:h-auto md:w-1/2 flex items-center justify-center">
          <img class="object-cover w-[300px] h-[300px]" src="../assets/taskminder_logo.png" alt="img" />
        </div>
        <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2 flex-grow">
          <div class="w-full">
            ...
            <!-- Pilihan Role -->
            <div class="mb-4">
              <label class="block text-sm flex">Pilih Peran Anda
                <p class="text-red-700">*</p>
              </label>
              <div class="flex gap-4">
                <button class="btn btn-neutral text-white" :class="{ 'btn-selected': selectedRoles.includes('guru') }"
                  @click="toggleRole('guru')" :disabled="isGuruRoleDisabled()">
                  Guru
                </button>
                <button class="btn btn-neutral text-white" :class="{ 'btn-selected': selectedRoles.includes('siswa') }"
                  @click="toggleRole('siswa')" :disabled="isSiswaRoleDisabled()">
                  Siswa
                </button>
                <button class="btn btn-neutral text-white"
                  :class="{ 'btn-selected': selectedRoles.includes('pengurus_kelas') }"
                  @click="toggleRole('pengurus_kelas')" :disabled="isPengurusKelasRoleDisabled()">
                  Pengurus Kelas
                </button>
              </div>
            </div>
            ...
            <div>
              <label class="block text-sm flex" v-if="isValidRegistration() && isGuruRoleDisabled()">NISN
                <p class="text-red-700">*</p>
              </label>
              <input v-if="isValidRegistration() && isGuruRoleDisabled()" id="nisn" name="nisn" type="text" required
                v-model="credentials.nisn"
                class="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600" />
            </div>

            <div>
              <label class="block text-sm flex mt-4" v-if="isValidRegistration() && isGuruRoleDisabled()">Nomor Absen
                <p class="text-red-700">*</p>
              </label>
              <input v-if="isValidRegistration() && isGuruRoleDisabled()" id="nomor_absen" name="nomor_absen"
                type="text" required v-model="nomor_absen"
                class="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600" />
            </div>
            ...
            <div>
              ...
              <select v-if="isValidRegistration() && isGuruRoleDisabled()" id="class_id" name="class_id"
                v-model="class_id" required
                class="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600">
            ...
            <div>
              <label class="block text-sm flex" v-if="isValidRegistration() && !isGuruRoleDisabled()">NIP
                <p class="text-red-700">*</p>
              </label>
              <input v-if="isValidRegistration() && !isGuruRoleDisabled()" id="nip" name="nip" type="text" required
                v-model="credentials.nip"
                class="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600" />
            </div>

            <div v-if="isValidRegistration() && !isGuruRoleDisabled()">
              <label class="block text-sm flex mt-4">Guru Mata Pelajaran
                <p class="text-red-700">*</p>
              </label>
              <select v-model="guru_mata_pelajaran" required
                class="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600">
                <option v-for="subjectOption in subjectOptions" :key="subjectOption" :value="subjectOption">
                  {{ subjectOption }}
                </option>
              </select>
            </div>
            ...
            <div>
              ...
              <input id="name" name="name" type="name" autocomplete="name" required placeholder="Nama"
            ...
            <div>
              ...
              <input id="email" name="email" type="email" autocomplete="email" required placeholder="johndoe@mail.com"
            ...
            <div>
              ...
              <input id="phone_number" name="phone_number" type="text" autocomplete="phone_number" required
                placeholder="0888888888888" v-model="credentials.phone_number"
                class="w-full px-4 py-2 text-sm border rounded-md focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-600" />
            </div>
            <div>
              <label class="block mt-4 text-sm flex">
            ...
            <div>
              ...
              <input id="password_confirmation" name="password_confirmation" type="password"
                autocomplete="current-password" required="" placeholder="Password Confirmation"
            ...
            <button v-if="!loading"
              ...
              @click="register" :disabled="!isValidRegistration">
              Register
            </button>
            ...
            <p class="text-blue-600 underline">
              <router-link :to="{ name: 'login' }">Sudah Punya Akun? Klik Disini Untuk Masuk</router-link>
            </p>
          </div>
        </div>
      </div>
